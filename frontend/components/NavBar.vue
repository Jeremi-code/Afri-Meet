<template>
  <nav class="sticky top-0 w-screen px-10 bg-white h-16 z-50 border-b border-gray-200">
    ...
    <transition name="slide-right">
      <div v-if="isMobileMenuOpen" class="fixed top-0 right-0 mlsm:w-screen w-1/3 msm:w-1/2 lsm:w-2/3  h-screen bg-white flex flex-col z-40 transition-transform duration-300 ease-in-out">
        ...
        <ul class="mt-[160px] space-y-4 flex-grow flex flex-col items-center">
          ...
          <div v-if="!isAuthenticated" class="flex flex-row space-x-4 my-6">
            <li>
              <UButton class="border-50" to="/register">
                Register
              </UButton>
